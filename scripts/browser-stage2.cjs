...
async function run() {
  ...
  assert.deepEqual((await store('meals')).find(m=>m.id===legacy.meal.id),legacy.meal);assert.deepEqual((await store('weights'))[0],legacy.weight);assert.deepEqual((await store('settings'))[0],legacy.settings);checks.push('real IndexedDB v1→v3 retains all legacy fields');
  ...
  await add();await method('外食');await page.getByRole('searchbox').fill('SUBWAY');await page.locator('.catalog-row').click();await page.getByText('メニューデータは今後追加予定です。',{exact:true}).waitFor();await close();checks.push('quick calories and restaurant placeholder without menu data');
