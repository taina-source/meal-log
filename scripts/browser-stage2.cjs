...
async function run() {
  ...
  await add();await method('外食');await page.getByRole('searchbox').fill('ガスト');await page.locator('.catalog-row').click();await page.getByText('メニューデータは今後追加予定です。',{exact:true}).waitFor();await close();checks.push('quick calories and restaurant placeholder without menu data');
