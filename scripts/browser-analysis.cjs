...
async function run() {
  ...
  await overview.getByText('記録日 4 / 7日 · 分析対象 4日',{exact:true}).waitFor();
  ...
  await seed('weight');await overview.getByText('記録日 0 / 7日 · 分析対象 0日',{exact:true}).waitFor();
  await page.getByText('分析対象日が少ないため傾向判定なし（各期間4日以上が必要です）。').waitFor();
