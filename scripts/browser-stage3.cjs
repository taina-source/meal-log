...
async function add() { await page.locator('nav').getByRole('button',{name:'食事を追加',exact:true}).click(); await page.getByRole('button',{name:'昼食',exact:true}).click(); await page.getByRole('button',{name:/^外食 21チェーン/}).click(); }
