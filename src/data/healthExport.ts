...
import { confirmedHealthMetadata, pendingHealthExport, supportedHealthBatch } from '../domain/healthExport';
...
export async function finishHealthExport(exportId: string, confirm: boolean) {
  return db.transaction('rw', db.settings, db.weights, async () => {
    ...
    if (confirm) for (const row of supportedHealthBatch(pending.payload).measurements) {
