...
import { chatgptModified, chatgptTotals, normalizeChatgptItem, validateInputType, type ChatgptItem, type ImportReceipt } from '../domain/chatgpt';
...
export async function registerChatgpt(receipt: ImportReceipt, edited: ChatgptItem[], context: { date: string; time: string; mealType: MealType }): Promise<MealEntry[]> {
  ...
  const inputType = validateInputType(receipt.payload.inputType);
  ...
  const entries = edited.map((draft, index): MealEntry => {
    ...
    return { ...chatgptTotals([item]), id: createId(), name: item.name, restaurant: item.restaurant,
      ...
      chatgptSnapshot: { ...snapshot, declaredSourceType, schemaVersion: 1, importedAt: receipt.importedAt, ...(inputType ? { inputType } : {}) }, chatgptUserModified: chatgptModified(original, item), chatgptModifiedBeforeSave: chatgptModified(original, item) };
  });
