import { useMealUndo } from './components/useMealUndo';
import { Suggestions } from './pages/Suggestions';
import { HealthShare } from './pages/HealthShare';
...
type Overlay = { kind: 'suggestions' } | { kind: 'health' } | { kind: 'chatgpt'; receipt: ImportReceipt; key: string } | { kind: 'meal'; type: MealType; date: string; entry?: MealEntry } | { kind: 'detail'; entry: MealEntry } | { kind: 'weight'; date: string } | { kind: 'copy'; date: string } | null;
...
export default function App({ initialImport = null }: { initialImport?: ImportReceipt | null }) {
  ...
  const mealUndo = useMealUndo(setToast);
  ...
  return <div className="app-shell"><header className="app-header"><a className="brand" href="#" onClick={event => { event.preventDefault(); navigate('home'); }} aria-label="Meal Log ホーム"><span className="brand-mark"><Icon name="leaf" size={21} /></span><span>Meal Log<span className="brand-dot">.</span></span></a><span className="header-caption">毎日を、ちょうどよく。</span></header><main id="main-content">{!settings ? <p className="loading">記録を読み込み中…</p> : <>{page === 'home' && <Home date={date} setDate={setDate} settings={settings} onAdd={add} onHistory={type => { setFilter({ date, type }); setPage('history'); window.scrollTo({ top: 0 }); }} onWeight={() => setOverlay({ kind: 'weight', date })} onHealth={() => setOverlay({ kind: 'health' })} onCopy={() => setOverlay({ kind: 'copy', date })} onSuggestions={() => setOverlay({ kind: 'suggestions' })} />}{page === 'history' && <History onDelete={mealUndo.remove} notify={setToast} settings={settings} filter={filter} onClearFilter={() => setFilter(null)} onSelect={entry => setOverlay({ kind: 'detail', entry })} onAdd={() => add(filter?.type, filter?.date)} />}{page === 'analysis' && <Analysis settings={settings} />}{page === 'settings' && <Settings settings={settings} notify={setToast} />}</>}</main><Navigation page={page} onNavigate={navigate} onAdd={() => add()} /><PwaStatus />{mealUndo.batch && <div className="toast undo-toast" role="status"><span>{mealUndo.batch.entries.length}件削除しました</span><button disabled={mealUndo.busy} onClick={mealUndo.undo}>元に戻す</button></div>}{toast && !mealUndo.batch && <div className="toast" role="status"><Icon name="check" size={18} />{toast}</div>}{overlay && settings && <Modal key={overlay.kind + (overlay.kind === 'chatgpt' ? overlay.key : '') + ('entry' in overlay ? overlay.entry?.id ?? '' : '')} title={overlay.kind === 'suggestions' ? '食事候補' : overlay.kind === 'health' ? 'ヘルスケアへ共有' : overlay.kind === 'chatgpt' ? 'ChatGPT取り込み' : overlay.kind === 'meal' ? overlay.entry ? '食事を編集' : '食事を追加' : overlay.kind === 'detail' ? '食事の詳細' : overlay.kind === 'copy' ? '前日の食事をコピー' : '身体測定を記録'} onClose={() => setOverlay(null)}>{overlay.kind === 'meal' && (overlay.entry ? <MealForm date={overlay.date} initialType={overlay.type} entry={overlay.entry} onSaved={saved} /> : <PfcDecimalsContext.Provider value={settings.showPfcDecimals}><AddMealFlow date={overlay.date} initialType={overlay.type} onSaved={saved} /></PfcDecimalsContext.Provider>)}{overlay.kind === 'chatgpt' && <PfcDecimalsContext.Provider value={settings.showPfcDecimals}><ChatgptImport initialReceipt={overlay.receipt} onSaved={saved} onCancel={() => setOverlay(null)} /></PfcDecimalsContext.Provider>}{overlay.kind === 'copy' && <CopyMeals date={overlay.date} onDone={message => { setOverlay(null); setToast(message); }} />}{overlay.kind === 'detail' && <MealDetail entry={overlay.entry} settings={settings} onEdit={() => setOverlay({ kind: 'meal', date: localDate(new Date(overlay.entry.eatenAt)), type: overlay.entry.mealType, entry: overlay.entry })} onDelete={mealUndo.remove} onDeleted={() => setOverlay(null)} />}{overlay.kind === 'suggestions' && <PfcDecimalsContext.Provider value={settings.showPfcDecimals}><Suggestions key={today} today={today} settings={settings} onSaved={saved} onSettings={() => { setOverlay(null); navigate('settings'); }} /></PfcDecimalsContext.Provider>}{overlay.kind === 'health' && <HealthShare />}{overlay.kind === 'weight' && <WeightForm date={overlay.date} onSaved={() => { setOverlay(null); setToast('身体測定を保存しました'); }} />}</Modal>}</div>;
}
