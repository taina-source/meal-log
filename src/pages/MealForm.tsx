...
export function MealForm({ date, initialType, initialTime, entry, onSaved }: { date: string; initialType: MealType; initialTime?: string; entry?: MealEntry; onSaved: (date: string) => void }) {
  ...
  return <form className="form-stack" noValidate onSubmit={submit}>
    ...
    <div><p className="eyebrow">MANUAL ENTRY</p><h3>手動で食事を追加</h3><p className="help">わかる栄養素だけで大丈夫です。</p></div>
    ...
    <NumberField label="カロリー" unit="kcal" value={calories} onChange={setCalories} required max={20000} />
    <div className="form-pfc"><NumberField label="P たんぱく質" unit="g" value={protein} onChange={setProtein} /><NumberField label="F 脂質" unit="g" value={fat} onChange={setFat} /><NumberField label="C 炭水化物" unit="g" value={carbs} onChange={setCarbs} /></div>
    <p className="help compact">PFCは空欄の場合、0gで保存します。</p>
