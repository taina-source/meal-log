...
import { healthCounts, healthMeasurements, healthRange, healthShortcutUrl, isHealthShared, type HealthScope } from '../domain/healthExport';
import { measurementFields, measurementSpec, measurementValues } from '../domain/measurements';
...
export function HealthShare() {
  ...
  return <div className="form-stack health-share"><p>未共有 {total.days}日分・{total.fields}項目</p>
    <p className="help">直接同期ではなく「Meal Log Health」ショートカット経由の記録です。初回はショートカット側でヘルスケア書き込み許可が必要です。</p>
    ...
    {pending && pendingCount ? <section className="form-stack" aria-label="共有確認待ち"><h3>ヘルスケアへの共有確認待ち</h3><p>{pendingCount.days}日分・{pendingCount.fields}項目</p><p className="help">前回の共有確認待ちがあります。完了またはキャンセルするまで新しい共有は開始できません。</p>
      ...
      <ActionButton action={async () => { if (await copy()) window.location.href = healthShortcutUrl; }}>ショートカットで共有</ActionButton>
      <ActionButton className="button secondary" action={copy}>連携用JSONをコピー</ActionButton>
      <a className="button secondary" href={healthShortcutUrl}>Meal Log Healthを開く</a>
      <ActionButton className="button secondary" action={async () => { if (await copy()) window.location.href = healthShortcutUrl; }}>もう一度ショートカットを開く</ActionButton>
    ...
    <details><summary>日別の共有状態</summary>{[...data.entries].sort((a, b) => b.date.localeCompare(a.date)).map(entry => <div key={entry.id}><h3>{entry.date}</h3>{measurementFields.map(field => { const value = measurementValues(entry)[field]; return value === undefined ? null : <p key={field}>{measurementSpec[field].label} {formatNumber(value, true)}{measurementSpec[field].unit} · {isHealthShared(entry, field) ? '共有済み' : '未共有'}</p>; })}</div>)}</details>
  </div>;
}
