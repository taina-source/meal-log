...
import { normalizeSectionOrder, defaultSectionOrder, sectionLabels, moveSection, type AnalysisSection } from '../domain/analysisDisplay';
...
import { analysisPolicy, isAnalysisDay, analysisRange, dailyMeals, dailyWeights, mealSummary, recentTrends, targetDifference, weightSummary, type AnalysisPeriod } from '../domain/analysis';
...
export function Analysis({ settings }: { settings: UserSettings }) {
  const [editing,setEditing] = useState(false), [saving,setSaving] = useState(false), [displayError,setDisplayError] = useState('');
  const order = normalizeSectionOrder(settings.analysisSectionOrder);
  async function persistOrder(next: AnalysisSection[]) {
    setSaving(true);setDisplayError('');
    try { await saveAnalysisDisplay({order:next}); } catch(error) { setDisplayError(storageError(error)); } finally { setSaving(false); }
  }
  ...
  const trends = recentTrends(mealDays, weightDays, today, policy);
  ...
  const cards: Partial<Record<AnalysisSection, ReactNode>> = data && !data.error ? {
    maintenance: (<MaintenanceCard meals={mealDays} weights={weightDays} policy={policy} />),
    calories: (<section className="card analysis-block" aria-label="摂取カロリー概要"><h2>摂取カロリー概要</h2>
        ...
        <p className="analysis-note">平均は分析対象日のみ。未記録日と設定で除外された日は含めません。比較は過去の目標ではなく、現在の設定値です。</p>
      </section>),
    pfc: (<section className="card analysis-block" aria-label="PFC概要"><h2>平均PFC</h2><div className="analysis-pfc">{pfc.map(({ key, label, target }) => <div key={key}><h3>{label}</h3><b>{meals.average ? formatNumber(meals.average[key], decimals) : '—'} g</b><span>現在目標 {formatNumber(target, decimals)} g</span><span>差 {difference ? `${signed(difference[key], decimals)} g` : '—'}</span></div>)}</div></section>),
    intake: (<section className="card analysis-block"><h2>摂取カロリー・PFC推移</h2><IntakeChart records={meals.records} range={range} settings={settings} />
        <p className="analysis-note">未記録日はデータなし。線をつながず、0 kcalとして扱いません。白抜きの点は平均対象外（記録は有効）です。</p>
        {!!meals.records.length && <details><summary>カロリーの日別データ</summary><table><caption>食事記録のある日の合計</caption><thead><tr><th>日付</th><th>kcal</th></tr></thead><tbody>{meals.records.map(day => <tr key={day.date}><th scope="row">{shortDate(day.date)}</th><td>{formatNumber(day.calories)}{!isAnalysisDay(day, policy) && <small>平均対象外</small>}</td></tr>)}</tbody></table></details>}
      </section>),
    weight: (<section className="card analysis-block" aria-label="体重概要"><h2>体重</h2>{weight.latest && weight.first ? <>
    ...
    weightChart: (<section className="card analysis-block"><h2>体重推移</h2><AnalysisChart title="実測体重と7日移動平均" range={range} unit="kg" series={[
    ...
    relationship: (<RelationshipCard meals={mealDays} weights={weightDays} range={range} policy={policy} />),
    bodyFat: (<MeasurementAnalysis entries={data.weights} range={range} part="bodyFatPercent" />),
    waist: (<MeasurementAnalysis entries={data.weights} range={range} part="waistCm" />),
    measurements: (<MeasurementAnalysis entries={data.weights} range={range} part="table" />),
    trends: (<section className="card analysis-block" aria-label="最近の傾向"><h2>最近の傾向</h2><p className="analysis-note">選択期間にかかわらず、今日を含む直近7日とその前の7日を比較します。</p>
  ...
  return <div className="analysis-page">
    ...
    <div className="analysis-arrange"><button className="button secondary" aria-pressed={editing} onClick={()=>setEditing(!editing)}>{editing?'並び替えを完了':'並び替え'}</button>{editing && <button className="button secondary" disabled={saving} onClick={()=>persistOrder(defaultSectionOrder)}>初期順に戻す</button>}</div>
    ...
    {!data ? <p role="status">記録を読み込み中…</p> : data.error ? <p role="alert">記録を読み込めませんでした。画面を開き直してください。</p> : <>
      ...
      {order.map((id,index)=><div key={id} className="analysis-section" data-section={id}>
        {editing && <div className="analysis-reorder"><span>{sectionLabels[id]}</span><button className="button secondary" aria-label={sectionLabels[id]+'を上へ'} disabled={saving || index===0} onClick={()=>persistOrder(moveSection(order,id,-1))}>上へ</button><button className="button secondary" aria-label={sectionLabels[id]+'を下へ'} disabled={saving || index===order.length-1} onClick={()=>persistOrder(moveSection(order,id,1))}>下へ</button></div>}
        {cards[id]}
      </div>)}
