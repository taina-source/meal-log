import { PhotoImportInstructions, PhotoEstimateNotice } from '../../components/PhotoImportInstructions';
...
export function ChatgptImport({ initialReceipt, context, onSaved, onCancel, mode = 'text' }: { mode?: 'text' | 'photo'; initialReceipt?: ImportReceipt; context?: { date: string; time: string; mealType: MealType }; onSaved: (date: string) => void; onCancel: () => void }) {
  ...
  return <div className="form-stack chatgpt-import"><h3>{mode === 'photo' ? '写真からChatGPT取り込み' : 'ChatGPTから取り込み'}</h3><p className="help">内容を確認・修正してから保存します。登録するまで端末の記録は変わりません。</p>{!receipt?.payload ? <>
    {mode === 'photo' && <PhotoImportInstructions />}
    <button className="button secondary" disabled={reading} onClick={clipboard}>{reading ? '読み込み中…' : mode === 'photo' ? 'クリップボードから結果を読み込む' : 'クリップボードから読み込む'}</button>
    ...
    {receipt.payload.inputType === 'photo' && <PhotoEstimateNotice />}
    {mode === 'photo' && receipt.payload.inputType !== 'photo' && <p className="catalog-note">このJSONはテキスト形式です（inputType未指定／text）。写真の結果なら、写真用プロンプトでJSONを出力し直してください。</p>}
    ...
    {items.map((item, index) => <fieldset className="chatgpt-item form-stack" key={index} disabled={busy}><legend>商品{index + 1}</legend>
      ...
      <ChatgptSource item={item} inputType={receipt.payload!.inputType} />{chatgptModified(receipt.payload!.items[index], item) && <p className="help">ユーザー修正あり（元の値も保存します）</p>}{chatgptWarnings(item).map(warning => <p className="catalog-note" key={warning}>{warning}</p>)}
    </fieldset>)}
