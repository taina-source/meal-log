...
export function Home({ date, setDate, settings, onAdd, onHistory, onWeight, onComingSoon }: { date: string; setDate: (date: string) => void; settings: UserSettings; onAdd: (type?: MealType) => void; onHistory: (type: MealType) => void; onWeight: (weight?: number) => void; onComingSoon: () => void }) {
  ...
  return <>
    ...
    <section className="meal-section"><div className="section-heading"><h2>{today ? '今日の食事' : 'この日の食事'}</h2><span>{meals?.length ?? 0}件の記録</span></div><div className="card meal-list">{mealTypes.map(type => {
