...
export function MeasurementAnalysis({ entries, range, part }: { entries: WeightEntry[]; range: DateRange; part?: 'bodyFatPercent' | 'waistCm' | 'table' }) {
  const rows = dailyMeasurements(entries).filter(row => row.date >= range.start && row.date <= range.end);
  return <>{(['bodyFatPercent', 'waistCm'] as const).filter(field => !part || part === field).map(field => {
