...
export function MenuNutrition({ item }: { item: RestaurantMenuItem }) {
  ...
  return <div className="menu-nutrition">{item.servingBasis && <span className="serving-basis">{item.servingBasis}</span>}<strong>{number(item.calories)} kcal</strong><span>P {number(item.protein, true)} / F {number(item.fat, true)} / C {number(item.carbs, true)} g</span>{[item.protein, item.fat, item.carbs].includes(null) && <span className="catalog-note">PFC情報なし（一部または全部不明）</span>}</div>;
}
export function MenuProvenance({ item }: { item: RestaurantMenuItem }) {
  return <section className="form-stack provenance"><h3>栄養情報・出典</h3><p className="help">{item.servingBasis ?? '表示の1単位'}当たり。数量を掛けて食事に保存します。</p>{nutrientKeys.map((key, index) => {
