# ChatGPT → Meal Log：第3B-1 テキスト取り込み

ChatGPTアプリの**テキスト版「Ask ChatGPT」**から返されたJSONを、iPhoneショートカット経由でMeal Logへ渡します。Meal LogはChatGPT APIを使用せず、APIキー・独自サーバー・追加の従量課金はありません。契約済みChatGPT Plusのアプリを使う想定で、ChatGPT側の利用制限は別途適用されます。
...
6. **「URLエンコード」**を追加し、「エンコード」を選択。入力はChatGPTの回答テキスト。JSONだけを**1回**エンコードします。
7. **「テキスト」**を追加。次の固定部分の直後に、URLエンコードの結果変数を挿入します。改行・空白は入れません。

   ```text
   https://taina-source.github.io/meal-log/#ml-import=【URLエンコードの結果】
   ```

8. **「URLを開く」**へ上のテキストを渡します。URL全体を再度エンコードしないでください。「URLの内容を取得」やAPIへのリクエストは不要です。
9. ショートカットを実行します。Meal Logで確認画面が出たら、商品名・店名・数量・単位・1単位のkcal/PFCを確認／編集し、食事区分・日時を選び、**「登録する」**を押します。
10. ホームの日次合計と履歴で結果を確認してください。履歴詳細の「元入力情報」から元のChatGPT値を見られます。

**Safariとホーム画面PWAの確認**：HTTPSの「URLを開く」がインストール済みPWAを直接開くか、Safariタブを開くかは端末・iOSの挙動に依存し、アプリから強制できません。最初の1件で保存先が普段使っているMeal Logと同じかを実機確認してください。別の保存領域で開く場合は、その画面では登録せず、回答JSONをコピーして普段のホーム画面PWAの「ChatGPTから取り込み」へ貼り付けてください。自動でクラウド同期や保存領域移行は行いません。
...
食事写真・画像版Ask ChatGPTの自動受け取りは第3B-2以降です。OpenAI API、OCR、バーコード、外部栄養API、CSV/JSONの全データExport/Import、同期、ログイン、バックエンドは追加していません。ここでのJSON取り込みは食事候補の確認登録専用です。
