# Meal Log — 第4A：分析画面の基礎

既存の分析タブに、7日・30日・90日・全期間の食事／PFC／体重集計とSVGグラフを追加しました。新しいdependency、API、DB table/indexは追加していません。

- 7/30/90日は端末ローカル日付の今日を含む暦日。全期間は保存された食事・体重の最初の日〜最後の日です（保存済みの未来日も含みますが、未来を予測・補間しません）。
- kcal/P/F/C平均の分母は食事を1件以上記録した日数です。未記録日は0として扱いません。記録された0kcalは記録日に含みます。比較対象はすべて**現在の目標**です。過去の目標履歴はありません。PFC小数表示は既存設定を使用します。
- カロリー推移に現在目標の参考線、体重推移に実測と7日移動平均を表示。未記録日は線を切り、日別の値は折りたたみ表でも確認できます。
- 体重保存は既存の1日1件・同日上書きを維持。集計関数へ同日複数件を渡した場合はcreatedAt最新（同時刻はID順）の1件を採用します。
- 7日移動平均は各体重記録日を含む過去7暦日内の実測だけで計算し、使用した記録日数を併記します。1件から計算可能ですが、7日分が揃った意味ではありません。期間先頭では直前6日分も参照し、未記録日の点は生成しません。「最新」は期間内最新体重の記録日時点です。
- 直近7日 vs 前7日は期間切替とは独立。各期間4日以上の食事記録がある場合だけ平均差を表示し、不足時は傾向判定なし。体重は直近7日に2日以上記録がある場合だけ実測の変化量を表示します。
- IndexedDB v3、既存MealEntryの栄養値・snapshot、MEXT食品DB、21チェーン外食JSON、`/meal-log/`・Service Workerを維持。分析結果は保存せず、記録・設定の変更に応じて再集計します。

主な実装：`src/domain/analysis.ts`（pure function）、`src/pages/Analysis.tsx`、`src/components/AnalysisChart.tsx`、`src/styles/analysis.css`。対象テストは `pnpm test src/domain/analysis.test.ts`。ブラウザー検証は `scripts/browser-analysis.cjs`（隔離プロファイルに試験データのみ作成）。

検証結果：分析16件を含む全358件の自動テスト成功（最終全件実行1回）。`pnpm run build` 1回でTypeScript・本番build・PWA生成成功。precacheは37エントリー、20,534.17KiB。390×844／320px幅のライト・ダーク、縦スクロール、空データ・食事だけ・体重だけ・体重1件・期間切替・日別表を本番プレビューで確認し、横はみ出し・console/page errorは0件でした。実機確認は公開更新後に行ってください。

iPhoneでは期間切替、記録日数／現在目標との差、体重の期間変化と平均、日別表、ライト／ダークでの縦スクロールをご確認ください。

第4B以降へ残すもの：維持カロリー推定、摂取と体重の因果・相関分析、将来予測、食事提案。今回の「最近の傾向」は記録値の単純比較だけです。
