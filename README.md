...
維持カロリー推定は第4Bで実装済みです。摂取量との相関・因果分析、将来予測、食事提案、体脂肪率／ウエストの移動平均は未実装です。
...
第4A時点で次段階へ残した維持カロリー推定は、第4Bで実装済みです。摂取と体重の因果・相関分析、将来予測、食事提案は引き続き未実装です。「最近の傾向」は記録値の単純比較だけです。
