...
- **共有方式**：HealthKit直接アクセスや同期ではなく、別途作成する「Meal Log Health」ショートカット経由です。全期間／今日／過去7日／過去30日／任意期間をローカル暦日で選択。未共有判定は項目ごとに「現在の保存値＝最後に完了確認した値」で比較し、編集で値が変わると再び未共有です。空欄項目は送らず、Health側の既存値を削除しません。
- **batch**：常に `schemaVersion:1`、`type:"meal-log-health-batch"`、新規UUIDの`exportId`、日付昇順の`measurements:[{date:"YYYY-MM-DD",weightKg?,bodyFatPercent?,waistCm?}]`。必要なnumberだけを含め、null・0補完・食事情報は送りません。体脂肪率24%は`24`、ウエスト98cmは`98`です。ショートカット側で単位・日付を解釈します。
...
- **未確認・次の実機作業**：今回ショートカット本体は作成していません。実際のHealth書き込み、ウエスト周囲径の選択可否、体脂肪率の入力単位、日付の扱い、iPhone PWAでのclipboard後の自動起動は、ショートカット作成時に実機で確認してください。未作成でも身体測定・分析・未共有管理は利用できます。
