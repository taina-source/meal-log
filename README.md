...
現在のテキスト入力は **ChatGPTの「Meal Log専用プロジェクト」→ 食べた物を入力 → JSONをコピー → 普段のホーム画面Meal Logの「ChatGPTから取り込み」** が正式ルートです。テキスト版ショートカットは正式ルートから外し、Meal Logからショートカットを起動する機能は今後実装しません。
