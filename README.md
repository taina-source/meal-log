# Meal Log — 第5E.1：分析画面のカスタマイズ

摂取推移でkcal・P・F・Cを個別に表示切替できます（初期はkcalのみ）。混在時は左軸kcal／右軸g、PFCのみはg軸。単位・線種・点の形で系列を区別し、全OFF時は選択案内を表示します。保存済み日別合計を使い、未記録日の欠損・0g記録・平均対象外の表示を維持します。

「並び替え」から主要11セクションを上へ／下へ移動し、初期順に戻せます。期間切替は上部に固定。系列選択・順番はSettingsのoptional fieldへ差分保存し、再起動とJSONバックアップ／復元で保持します。不明・重複IDは除外し、欠けたカードは既定順で末尾へ追加。IndexedDB v3・table/index・既存集計仕様は維持、新規dependencyなし。

## 第5E：月カレンダー表示

Historyの「一覧／カレンダー」を切り替え、日曜始まりの月表示を確認できます（初期は一覧）。前月／次月／今月で移動し、日別の保存済み総kcalと身体測定の「測」markerを表示。未記録は空欄、0kcalの記録は0として区別します。今日には枠線と読み上げ用の現在日指定があります。

日付タップで検索・絞り込みを解除し、その日の全食事を既存一覧に表示します。カレンダー集計は検索条件や低カロリー分析除外の影響を受けません。切替時は複数選択を解除し、第5Dの検索・一括操作・Undoは一覧で維持します。記録変更はlive反映、集計のDB保存なし。IndexedDB v3・table/index・静的栄養DBは未変更、新規dependencyなし。バックアップリマインダーは第5Fへ残しています。

## 第5D：履歴検索・一括操作・削除Undo（当時の記録）

履歴で最終保存された料理名・店名を全期間横断検索できます。期間（全期間／過去7・30・90日／任意）、食事区分、入力元で絞り込み、新しい順に表示します。セット・外食などの既存情報を使って入力元を分類し、内部snapshot本文は検索しません。

選択モードで個別／表示中すべてを選び、食事区分・日付の一括変更または削除ができます。検索条件の変更で選択は解除。操作前に件数と変更内容を確認します。日付変更ではローカル時刻・秒・ミリ秒を保持し、DST等で時刻を保持できない場合は拒否します。栄養値・ID・snapshotは変更しません。

一括削除と1件詳細からの削除は、約15秒だけ「元に戻す」が使えます。直近1回の削除だけをメモリ上に保持し、reload／PWA終了後はUndoできません。元ID・MealEntry全体を復元し、ID衝突時は上書きせず中止します。一括変更・削除・Undoは各々単一transactionで、途中失敗は全体をrollbackします。お気に入りの独立snapshotは維持します。

IndexedDB v3・table/index・静的栄養DBは未変更、新規dependencyなし。永続ゴミ箱・複数段Undo・カレンダー・バックアップリマインダーは今回未実装です。

## 第5C：摂取カロリーと体重トレンド

Analysisの選択期間終了日から、重複しない完全な7暦日ブロックを生成します（余りは除外）。現在の食事分析設定による対象日が最低5日、体重が最低2記録日・間隔4日以上の週だけを使用。平均摂取kcalと既存Theil–Sen法の体重トレンド（kg/週）を比較し、欠損の0補完や補間はしません。

有効5週以上で散布図とSpearman順位相関を表示します。同順位は平均順位、分散0は係数なし。分析週数を併記し、5〜7週は参考として見る案内を表示します。週ごとの詳細も確認できます。相関は因果関係を示さず、医学的評価・将来予測・食事指示ではありません。維持カロリー参考線は省略しています。

計算結果のDB保存・新規依存はなく、IndexedDB v3を維持。体脂肪率・ウエストは相関計算に含めず、第4B・第5Bの計算仕様も未変更です。

## 第5B：身体測定の7日移動平均

体脂肪率・ウエストの推移にも、体重と共通の7暦日移動平均を追加しました。各指標の実測日だけに、その日を含む過去7暦日の実測平均を表示します。未記録を0にせず、補間・carry-forwardは行いません。最新平均には使用件数を併記し、1件の場合も7日分が揃った意味ではありません。表示期間先頭の直前6日も計算に使いますが、期間外の点は描画しません。

既存の実測・期間変化・日別表は維持します。平均は表示時だけ計算し、DBへ保存しません。IndexedDB v3・table/index・依存関係は未変更。医学的評価や将来予測ではなく、記録を平滑化する表示です。
...
- **3タイプ**：バランス重視／たんぱく質重視／いつもの食事から、各最大5候補。同一候補のセクション間重複を避けます。1〜3品の内容・量・合計栄養・追加後の残りを表示し、内部scoreや架空の適合率は表示しません。
- **計算**：追加後の不足と超過を正規化し、超過側だけ1.35倍。バランスのkcal/P/F/C重みは0.40/0.30/0.15/0.15、たんぱく質重視は0.30/0.45/0.15/0.10です。いつもの食事はバランスscoreにお気に入り0.85／recent 0.90／レシピ・セット0.95の最も強い1つだけを適用します。
- **量・探索**：履歴・お気に入りは保存済みの量と最終値を優先。MEXTは25g刻み・400gまで（保存済み量は保持）、レシピは既存の1食分・全体の1/2・1/3・1/4等、セットは既存の構成を使用。単品を先に評価し、最大24種類・各2量のpoolから2〜3品を探索します。同じsourceを重ねず、同点は品数・親しみのある項目・保存量・stable IDで決定します。
- **確認して登録**：候補を選ぶだけでは保存しません。確認画面で食事区分・日時・量を編集し、量の比率で再計算してから登録します。セットは構成品ごとに編集。食品・レシピ・手動・ChatGPTの既存保存処理を使い、過去記録やお気に入りsnapshotを変更しません。
- **外食**：「外食から探す」を開いた時だけ既存21チェーンJSONを読み込み、登録可能な完全栄養値の単品候補を表示。既存の詳細・出典・カートへ進みます。外食同士の自動組み合わせや店舗間組み合わせは作りません。不明値を0補完しません。
- **案内**：全目標達成時は候補を折りたたみ、希望時のみ表示。ほぼ達成時も中立的に案内します。食べることを指示する機能ではありません。
- **互換性**：IndexedDB v3、table/index/migration変更なし。MEXT・21外食JSON、ChatGPT import・Health共有仕様、`/meal-log/`・PWA・offlineを維持。新規dependencyなし。

## 第4B：実績ベース推定維持カロリー

Analysisに、最新体重記録日を終了日とした直近30暦日の実績から推定するカードを追加しました。7／30／90日・全期間の表示切替とは独立しています。

- 食事は既存の分析対象日フィルター（既定1500kcal以下除外、OFFも可）を再利用。対象日の平均だけを用い、最低21日の食事分析対象が必要です。最新体重日より後の食事は含めません。
- 体重は最低8記録日かつ最古〜最新の間隔21日以上。全日付ペアのkg/dayの中央値（Theil–Sen法）でトレンドを求めます。既存グラフの7日移動平均は変更していません。
- 平均摂取kcal − 体重トレンドkg/day × 7700kcal/kgで近似し、表示だけ50kcal単位に丸めて「約」と表示します。厳密なTDEE測定ではありません。
- 充足度は「データ不足／参考／十分」。食事24日以上・体重14日以上・体重記録間隔28日以上の全条件で「十分」とします。精度の保証や％の信頼度ではありません。
- 30日間の体重中央値に対して**週1.5%を超える体重変化**では「推定保留」。水分等の影響に備えた計算上のガードで、医学的危険判定ではありません。非有限値や0以下の計算結果も表示しません。
- 体脂肪率・ウエストは既存の補助表示だけを維持し、計算には使用しません。欠損の0補完・補間・保存済みsnapshot変更なし。DB v3・schema/table/index・食品DB・外食DB・ChatGPT取り込み・Health共有仕様は変更していません。

## 第4A.2：身体測定＋Appleヘルスケア共有

- **身体測定**：1日1件、体重（kg）・体脂肪率（%）・ウエスト（cm）はすべて任意で最低1項目が必要。同日編集は既存値を表示し、空欄にした項目はその日の記録から外します。上限は体重500kg・体脂肪率100%・ウエスト300cm、0以下／非有限値は不可。表示は小数1桁を基本とし、保存値は丸めません。
- **Home／分析**：Homeに指標別の最新値（表示日まで）と共有入口を追加。7／30／90日・全期間で体脂肪率とウエストの最新値・期間変化・推移・日別表を表示します。未記録を0にせず補間しません。体脂肪率の差はポイント表示。既存体重の7日移動平均は維持し、食事の低カロリー日除外は身体測定に適用しません。
- **共有方式**：Appleヘルスケアへ共有するのは体重・体脂肪率のみです。ウエストはMeal Log内だけで記録・分析し、未共有件数・batch・再共有・共有完了処理には含めません。既存のhealthExport.waistCmは削除せず無視し、旧pendingのウエストも共有時に除外します。HealthKit直接アクセスや同期ではなく、別途作成する「Meal Log Health」ショートカット経由です。全期間／今日／過去7日／過去30日／任意期間をローカル暦日で選択。未共有判定は項目ごとに「現在の保存値＝最後に完了確認した値」で比較し、編集で値が変わると再び未共有です。空欄項目は送らず、Health側の既存値を削除しません。
- **batch**：常に `schemaVersion:1`、`type:"meal-log-health-batch"`、新規UUIDの`exportId`、日付昇順の`measurements:[{date:"YYYY-MM-DD",weightKg?,bodyFatPercent?}]`。必要なnumberだけを含め、null・0補完・食事情報は送りません。体脂肪率24%は`24`です。ショートカット側で単位・日付を解釈します。
- **確認待ち**：「共有内容を確認」で今回のsnapshotをSettingsのoptional `pendingHealthExport`へ保存し、再起動後も保持。「ショートカットで共有」はJSONをclipboardへコピー後、`shortcuts://run-shortcut?name=Meal%20Log%20Health`を開きます。JSONはURLへ埋め込みません。コピー／起動の個別操作と、画面のJSONを手動コピーするfallbackも用意しています。
- **完了／キャンセル**：起動だけでは共有済みにしません。ユーザーが「今回の共有を完了にする」と明示した時だけpendingの値と確認時刻を項目別metadataへ保存します。現在値が変更されていれば未共有のままです。削除済みの元recordはskip。キャンセルはpendingだけを削除し、再試行は同じpayload／exportIdを使います。未解決pendingを新しい共有で上書きしません。
- **Health側の制約**：Meal LogはHealth側の書き込み結果・削除・編集・重複を確認しません。共有済みはMeal Log側のユーザー確認状態です。「この期間をすべて再共有」は警告付きの補助操作で、Health側に重複サンプルができる可能性があります。初回の書き込み許可はショートカット側で必要です。
- **DB互換**：IndexedDB **v3維持**、table/index/migration変更なし。既存`weights`の旧`weight`を読み取り時に互換処理し、新しい身体測定はoptional `weightKg`／`bodyFatPercent`／`waistCm`／`healthExport`で保存します。既存行の一括書換えはせず、ユーザーが編集した1日だけ更新します。MEXT・21外食JSONは未変更です。
- **iPhone実機確認済み**：Appleヘルスケアへの体重・体脂肪率の書き込みを確認しました。`bodyFatPercent`はショートカットの%単位で正常に記録できます。Meal Logの「ショートカットで共有」からJSONコピー→Meal Log Health起動、Meal Logへ戻って「今回の共有を完了にする」で未共有件数が減ることも確認済みです。ウエストはMeal Log内のみで記録・分析し、Healthには共有しません。

維持カロリー推定は第4B、残りkcal/PFCからの食事候補は第4Cで実装済みです。体脂肪率／ウエストの7日移動平均は第5Bで実装済みです。記録上の摂取量と体重トレンドの相関分析は第5Cで実装済みです。因果分析・将来予測は未実装です。

## 第4A.1：履歴・お気に入り拡張

「お気に入り・履歴」にChatGPT／手動入力を追加。ChatGPTは取り込み単位でまとめて再登録でき、手動入力・かんたん入力も再利用できます。登録前に日時・食事区分・最終保存値を確認／編集します。kcal/PFCは前回保存した合計値を使い、数量を再乗算しません。各料理の☆で独立snapshotをお気に入り保存し、元履歴の編集・削除や再登録時の編集はお気に入りへ反映しません。再登録は新しいMealEntry ID／ChatGPTグループIDを発行し、元の出典・snapshotを保持します。IndexedDB v3・table/index・食品DB・21外食DBは変更していません。

現在のテキスト入力は **ChatGPTの「Meal Log専用プロジェクト」→ 食べた物を入力 → JSONをコピー → 普段のホーム画面Meal Logの「ChatGPTから取り込み」** が正式ルートです。テキスト版ショートカットは正式ルートから外し、ChatGPTへ質問するためにMeal Logからショートカットを起動する機能は今後実装しません。
...
- kcal/P/F/C平均の分母は食事記録のある日のうち、現在の分析設定を満たす分析対象日数です。未記録日は0として扱いません。記録された0kcalは記録日に含みます。比較対象はすべて**現在の目標**です。過去の目標履歴はありません。PFC小数表示は既存設定を使用します。
...
- 直近7日 vs 前7日は期間切替とは独立。各期間4日以上の分析対象日がある場合だけ平均差を表示し、不足時は傾向判定なし。体重は直近7日に2日以上記録がある場合だけ実測の変化量を表示します。
- IndexedDB v3、既存MealEntryの栄養値・snapshot、MEXT食品DB、21チェーン外食JSON、`/meal-log/`・Service Workerを維持。分析結果は保存せず、記録・設定の変更に応じて再集計します。

### 第4A仕上げ：分析除外とかんたんPFC

- 設定に「低カロリー日を分析から除外」と「分析対象最低カロリー」を追加。初期値はON／1500 kcalで、**1500以下は除外、1501は対象**です。OFFなら従来どおり食事記録のある全日を平均に含めます。設定値は0〜20,000 kcalで変更でき、過去の分析にも現在値を使います。
- 記録日数／分析対象日数を分け、除外数を表示します。平均kcal/PFC・現在目標との差・前後7日の食事傾向だけに適用。全日除外なら平均は「—」。グラフは白抜きの点、日別表は「平均対象外」として残し、履歴・MealEntry・体重は変更しません。判定はpure functionの `analysisPolicy` / `isAnalysisDay` で共用できます。
- かんたん入力は「割合（%）」「グラム」「PFCを入力しない」を選択。初期は割合25/30/45%。割合は総カロリーに占めるエネルギー比で、kcal×%÷100をP/Cは4、Fは9で割ってgに換算します。内部丸め・100%への自動補正はしません。
- 各割合は0〜100%。合計は100%以外も登録可能で、100%から10ポイント超の差だけ軽い警告。グラム入力は4/9/4換算kcalと差を表示し、差が150kcal以上かつ総カロリーの20%以上なら警告します。いずれも警告自体では登録を禁止しません（負値・非数・既存の入力上限違反は拒否）。
- 有効な割合は入力欄を離れた時にSettingsへ保存し、次回に再利用。登録時にも食事と設定を同一トランザクションで確定します。グラム・PFCなしは保存済み割合を変更しません。PFCなしは従来どおり0g、PFC分析でも0gです。
- Settingsにoptionalな `analysisExcludeLowCalories` / `analysisMinimumCalories` / `quickPfcPercentages` を追加。旧設定は読み替えで既定値を適用し、移行や一括書換えはありません。DB v3維持。過去のPFC=0記録を補完・再計算せず、通常のmanual MealEntryとして最終kcal/PFCだけを保存します。

仕上げ最終確認：全369件のテスト成功（全件1回）、`pnpm run build` 1回でTypeScript・本番build・PWA生成成功。precacheは37エントリー、20,540.51KiB。前回成功済みの関連45件と390px／320pxの変更画面確認は再実行していません。IndexedDB v3の定義とMEXT・21外食JSONはHEADとの差分なし。
...
仕上げ前の検証記録：分析16件を含む全358件の自動テスト成功（最終全件実行1回）。`pnpm run build` 1回でTypeScript・本番build・PWA生成成功。precacheは37エントリー、20,534.17KiB。390×844／320px幅のライト・ダーク、縦スクロール、空データ・食事だけ・体重だけ・体重1件・期間切替・日別表を本番プレビューで確認し、横はみ出し・console/page errorは0件でした。実機確認は公開更新後に行ってください。

iPhoneでは期間切替、記録日数／現在目標との差、体重の期間変化と平均、日別表、ライト／ダークでの縦スクロールをご確認ください。

第4A時点で次段階へ残した維持カロリー推定は第4B、食事候補は第4Cで実装済みです。摂取と体重の因果・相関分析、将来予測は引き続き未実装です。「最近の傾向」は記録値の単純比較だけです。

## 第3B-2までの記録

以下のショートカット関連の開発・検証記録は過去のものです。現在の推奨運用は「正式なiPhone運用」を参照してください。
...
- **テキスト**：ChatGPTの「Meal Log専用プロジェクト」→ 食べた物を入力 → 回答JSONをクリップボードへコピー → 普段のホーム画面Meal Log → ＋ → **ChatGPTから取り込み**。
- **写真**：ChatGPTアプリ／Meal Log専用プロジェクトへ写真を添付 → 写真用プロンプトと量の補足を送信 → 回答JSONをコピー → 普段のホーム画面Meal Log → ＋ → **写真からChatGPT取り込み**。
...
- **[テキスト版ショートカットの手順](docs/CHATGPT_SHORTCUT.md)**（過去の開発記録。現在の正式ルートではありません）
